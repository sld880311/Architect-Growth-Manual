...
- [Java中的调度方式详解](#java中的调度方式详解)
    - [抢占式调度](#抢占式调度)
    - [协同式调度](#协同式调度)
    - [JVM 的线程调度实现（抢占式调度）](#jvm-的线程调度实现抢占式调度)
    - [线程让出 cpu 的情况](#线程让出-cpu-的情况)
    - [进程调度算法](#进程调度算法)
        - [优先调度算法](#优先调度算法)
            - [先来先服务调度算法（FCFS）](#先来先服务调度算法fcfs)
            - [短作业(进程)优先调度算法](#短作业进程优先调度算法)
        - [高优先权优先调度算法](#高优先权优先调度算法)
            - [非抢占式优先权算法](#非抢占式优先权算法)
            - [抢占式优先权调度算法](#抢占式优先权调度算法)
        - [高响应比优先调度算法](#高响应比优先调度算法)
        - [基于时间片的轮转调度算法](#基于时间片的轮转调度算法)
            - [时间片轮转法](#时间片轮转法)
...
![1588842599605.png](..\images\1588842599605.png)
...
![1588843436863.png](..\images\1588843436863.png)
